...
function Sidebar({ showMenu, setShowMenu, node, name, navLinks }) {
  ...
  const activeClass = (path) => {
    ...
    if (router.pathname === path) {
      return "bg-[#E0EAFB] text-[#6495ED] rounded-md";
    }
    // if (router.pathname.includes(path)) {
    //   return "bg-custom-blue5 text-white";
    // }

    return "text-black hover:text-[#6495ED] hover:bg-[#E0EAFB] rounded-md";
  };
  ...
  return (
    ...
    >
      <div
        ref={node}
        className={`h-screen overflow-y-auto bg-white z-30 top-0 bottom-0 fixed lg:sticky sidebar text-black lg:translate-x-0 w-[264px] ease-out duration-300 ${
            ...
            {navLinks?.map((navLink, i) => (
              <div key={i} className="">
                <Link href={navLink.link} passHref>
                  <div
                    key={i}
                    className={`px-5 py-4 flex justify-between items-center font-semibold transition duration-300 rounded-sm ${activeClass(
                      navLink.link
                    )}`}
                  >
